...
function [y,ind,ind_max] = max_movel(signal, windowlength, overlap, zeropad, valor)
...
for i = indices
	index = index+1;
	[y(index),ind(index)] = max(signal(i:i+windowlength-1));
    ind(index) = ind(index) + i;
    ind_max_temp = find(signal(i:i+windowlength-1) >= valor*y(index)) + i;
    ind_max = [ind_max ind_max_temp]; clear ind_max_temp
end
